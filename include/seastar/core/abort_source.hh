...
class abort_source {
    using subscription_callback_type = noncopyable_function<void (std::exception_ptr) noexcept>;
...
public:
    ...
    class subscription : public bi::list_base_hook<bi::link_mode<bi::auto_unlink>> {
        ...
        explicit subscription(abort_source& as, naive_subscription_callback_type naive_cb, naive_cb_tag)
                : _target([cb = std::move(naive_cb)] (std::exception_ptr) noexcept { cb(); }) {
            as._subscriptions->push_back(*this);
        }
...
public:
    ...
    optimized_optional<subscription> subscribe(Func&& f) noexcept(std::is_nothrow_move_constructible_v<Func>) {
        ...
        if constexpr (std::is_invocable_v<Func, std::exception_ptr>) {
            ...
            return { subscription(*this, std::forward<Func>(f), subscription::naive_cb_tag{}) };
        }
    }
    ...
    void request_abort(std::exception_ptr ex = std::make_exception_ptr(abort_requested_exception())) noexcept {
    ...
    void request_abort(Exception&& e) noexcept {
        request_abort(std::make_exception_ptr(std::forward<Exception>(e)));
    }
