...
public:
    ...
    capacity_t maximum_capacity() const noexcept { return _token_bucket.burst_limit(); }
    void replenish_capacity(clock_type::time_point now) noexcept;

    std::chrono::duration<double> rate_limit_duration() const noexcept {
        std::chrono::duration<double, rate_resolution> dur((double)_token_bucket.burst_limit() / _token_bucket.rate());
