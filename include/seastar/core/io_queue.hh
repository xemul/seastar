...
#include <seastar/util/stairs.hh>
#include <mutex>
#include <array>
...
public:
    ...
    struct config {
        ...
        util::stairs<unsigned, 3> disk_bytes_write_multiplier = util::stairs<unsigned, 3>(read_request_base_count);
        util::stairs<unsigned, 3> disk_bytes_read_multiplier = util::stairs<unsigned, 3>(read_request_base_count);
...
inline size_t io_queue::capacity() const {
    return _group->_config.capacity;
}

inline sstring io_queue::mountpoint() const {
    return _group->_config.mountpoint;
}

inline dev_t io_queue::dev_id() const noexcept {
    return _group->_config.devid;
}
