...
class shard_aware_token_bucket {
    ...
    static T accumulated(T rate, rate_resolution delta) noexcept {
        return T(std::round(rate * delta.count()));
    }
...
public:
    ...
    T burst_limit() const noexcept { return _burst_limit; }
    ...
    shard_aware_token_bucket(T rate, T burst_limit, T threshold,
        ...
        , _tau_tokens(accumulated(std::min(rate, max_rate),
                                  std::min(std::chrono::duration_cast<rate_resolution>(tau), max_delta)))
