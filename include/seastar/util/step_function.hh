...
template <typename T, unsigned Inline = 2, typename Index = size_t>
SEASTAR_CONCEPT( requires std::is_arithmetic_v<T> && std::is_arithmetic_v<Index> )
class step_function {
    boost::container::small_vector<std::pair<Index, T>, Inline> _steps;
    template <typename U, unsigned V>
    friend std::ostream& operator<<(std::ostream&, const step_function<U, V>&);

public:
    /// Constructs the step_function with \c def as the default value
    step_function(T def) noexcept {
        _steps.emplace_back(std::numeric_limits<Index>::max(), def);
    }

    /// Returns the value corresponding to \c x
    T operator()(Index x) const noexcept {
        for (auto&& s : _steps) {
            if (x <= s.first) {
                return s.second;
            }
...
inline std::ostream& operator<<(std::ostream& out, const step_function<T, Inline>& st) {
    out << "step_function[";
