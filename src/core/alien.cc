...
void message_queue::start() {
    ...
    _metrics.add_group("alien", {
        ...
        sm::make_absolute("total_received_messages", _received, sm::description("Total number of received messages")),
        // total_operations value:DERIVE:0:U
        sm::make_absolute("total_sent_messages", [this] { return _sent.value.load(); }, sm::description("Total number of sent messages")),
    });
}
