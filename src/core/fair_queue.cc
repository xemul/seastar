...
again:
    ...
    if (extra >= _replenish_threshold) {
        if (!_replenished.compare_exchange_strong(ts, ts + std::chrono::duration_cast<std::chrono::nanoseconds>(delta))) {
            goto again;
        }

        auto max_extra = wrapping_difference(_capacity_top.load(std::memory_order_relaxed), _capacity_head.load(std::memory_order_relaxed));
