...
class io_desc_read_write final : public io_completion {
    ...
    const internal::io_direction_and_length _dnl;
    const stream_id _stream;
    fair_queue_ticket _fq_ticket;
    promise<size_t> _pr;

public:
    io_desc_read_write(io_queue& ioq, io_queue::priority_class_data& pc, internal::io_direction_and_length dnl, stream_id stream, fair_queue_ticket ticket)
        : _ioq(ioq)
        , _pclass(pc)
