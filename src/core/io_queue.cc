...
public:
    ...
    void dispatch(std::chrono::steady_clock::time_point queued, fair_queue_ticket x_ticket) noexcept {
        auto now = std::chrono::steady_clock::now();
        _pclass.on_dispatch(_dnl.len, std::chrono::duration_cast<std::chrono::duration<double>>(now - queued));
        _dispatched = now;
        _fq_ticket = x_ticket;
    }
...
public:
    ...
    {
        io_log.trace("dev {} : req {} queue  len {} ticket {}", _ioq.dev_id(), fmt::ptr(&*_desc), dnl.len, _ticket);
    }
...
void
io_queue::notify_request_finished(fair_queue_ticket x_ticket, internal::io_direction_and_length dnl) noexcept {
    _mixer.dec(dnl);
    _fq.notify_request_finished(x_ticket);
}
...
fair_queue_ticket io_group::request_fq_ticket(internal::io_direction_and_length dnl) const noexcept {
    ...
    if (dnl.dir) {
        ...
        size = _config.disk_bytes_write_multiplier(dnl.len) * dnl.len;
    } else {
        weight = io_queue::read_request_base_count;
        size = _config.disk_bytes_read_multiplier(dnl.len) * dnl.len;
    }
...
io_queue::queue_request(const io_priority_class& pc, size_t len, internal::io_request req, io_intent* intent) noexcept {
    return futurize_invoke([&pc, len, req = std::move(req), this, intent] () mutable {
        ...
        auto queued_req = std::make_unique<queued_io_request>(std::move(req), *this, pclass,
                internal::io_direction_and_length{ *direction, len });
