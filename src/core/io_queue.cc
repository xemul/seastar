...
fair_group::config io_group::make_fair_group_config(const io_queue::config& qcfg) noexcept {
    ...
    cfg.max_size = qcfg.max_bytes_count >> io_queue::request_ticket_size_shift;
    return cfg;
}
...
public:
    ...
    void tick() {
        if (_queue._queued_requests != 0 || _queue._requests_executing != 0) {
            ...
            if (_last.size() < _queue._priority_classes.size()) {
                _last.resize(_queue._priority_classes.size());
            }
            for (unsigned pc_i = 0; pc_i < _queue._priority_classes.size(); pc_i++) {
                const auto& pc = _queue._priority_classes[pc_i];
                auto& prev = _last[pc_i];
                if (pc) {
                    message += fmt::format(" [ pc.{} q:{} x:{} t:{}+{} {}+{}]", pc_i, pc->_nr_queued, pc->_nr_executing,
                        pc->_rwstat[0].ops - prev.ops[0],
                        pc->_rwstat[1].ops - prev.ops[1],
                        pc->_rwstat[0].bytes - prev.bytes[0],
                        pc->_rwstat[1].bytes - prev.bytes[1]);

                    prev.ops[0] = pc->_rwstat[0].ops;
                    prev.ops[1] = pc->_rwstat[1].ops;
                    prev.bytes[0] = pc->_rwstat[0].bytes;
                    prev.bytes[1] = pc->_rwstat[1].bytes;
                }
            }
