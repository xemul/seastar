...
public:
    io_desc_read_write(io_queue& ioq, priority_class_data& pc, internal::io_direction_and_length dnl)
        : _ioq(ioq)
        , _pclass(pc)
        , _dnl(dnl)
    {}
    ...
    void dispatch(std::chrono::steady_clock::time_point queued, fair_queue_ticket x_ticket) noexcept {
        auto now = std::chrono::steady_clock::now();
        _pclass.on_dispatch(_dnl.length(), std::chrono::duration_cast<std::chrono::duration<double>>(now - queued));
        _dispatched = now;
        _fq_ticket = x_ticket;
    }
...
public:
    queued_io_request(internal::io_request req, io_queue& q, priority_class_data& pc, internal::io_direction_and_length dnl)
        ...
        , _ticket(_ioq.request_fq_ticket_for_queue(dnl))
        , _desc(std::make_unique<io_desc_read_write>(_ioq, pc, dnl))
    {
        io_log.trace("dev {} : req {} queue  len {} ticket {}", _ioq.dev_id(), fmt::ptr(&*_desc), dnl.length(), _ticket);
    }
    ...
    void dispatch(fair_queue_ticket x_ticket) noexcept {
        ...
        _desc->dispatch(_started, x_ticket);
    ...
    fair_queue_ticket ticket_for_dispatch() const noexcept {
        return is_cancelled() ? fair_queue_ticket{} :
            _ioq.request_fq_ticket_for_dispatch(_desc->get_direction_and_length(), _ticket);
    }
...
void io_mixer::inc(io_direction_and_length dnl) noexcept {
    if (dnl.is_read()) {
        _group._beancounter.fetch_add(dnl.length());
    } else if (_writes++ == 0) {
...
void io_mixer::dec(io_direction_and_length dnl) noexcept {
    if (dnl.is_read()) {
        _group._beancounter.fetch_sub(dnl.length());
    } else if (--_writes == 0) {
...
fair_queue_ticket io_queue::request_fq_ticket_for_queue(internal::io_direction_and_length dnl) const noexcept {
...
fair_queue_ticket io_group::request_fq_ticket(internal::io_direction_and_length dnl) const noexcept {
    ...
    if (dnl.is_write()) {
        weight = get_config().disk_req_write_multiplier;
        size = get_config().disk_bytes_write_multiplier(dnl.length()) * dnl.length();
    } else {
        weight = io_queue::read_request_base_count;
        size = get_config().disk_bytes_read_multiplier(dnl.length()) * dnl.length();
    }

    return make_ticket(weight, size, dnl.length());
}
...
fair_queue_ticket io_group::request_fq_mixed_ticket(internal::io_direction_and_length dnl, fair_queue_ticket q_ticket) const noexcept {
    if (dnl.is_write() || !_mixer.has_writes()) {
        return q_ticket;
    }

    unsigned weight = io_queue::read_request_base_count;
    size_t size = get_config().disk_bytes_mixed_read_multiplier(_mixer.get_reads()) * dnl.length();
    ...
    if (size >= get_config().max_bytes_count) {
        if (size > oversize_warning_threshold) {
            ...
            io_log.warn("oversized request (length {}) submitted. "
                ...
                get_config().max_bytes_count >> io_queue::request_ticket_size_shift);
        }
...
io_queue::request_limits io_queue::get_request_limits() const noexcept {
    request_limits l;
    l.max_read = align_down<size_t>(std::min<size_t>(get_config().disk_read_saturation_length, get_config().max_bytes_count / get_config().disk_bytes_read_multiplier.get_default()), minimal_request_size);
    l.max_write = align_down<size_t>(std::min<size_t>(get_config().disk_write_saturation_length, get_config().max_bytes_count / get_config().disk_bytes_write_multiplier.get_default()), minimal_request_size);
    return l;
}
