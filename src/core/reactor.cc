...
future<size_t>
reactor::do_sendmsg(pollable_fd_state& fd, std::span<iovec> iovs, size_t iovlen) {
    return writeable(fd).then([this, &fd, iovs, iovlen] () mutable {
        msghdr mh = {};
        mh.msg_iov = iovs.data();
        mh.msg_iovlen = std::min<size_t>(iovs.size(), IOV_MAX);
        auto r = fd.fd.sendmsg(&mh, MSG_NOSIGNAL);
        if (!r) {
            return do_sendmsg(fd, iovs, iovlen);
        }
        if (size_t(*r) == iovlen) {
            fd.speculate_epoll(EPOLLOUT);
        }
...
future<size_t> pollable_fd_state::write_some(std::vector<iovec>& iov) {
    return engine()._backend->sendmsg(*this, std::span(iov), internal::iovec_len(iov));
}
#else
...
#if SEASTAR_API_LEVEL >= 9
future<> pollable_fd_state::write_all(std::vector<iovec>& iov) {
    return write_some(iov).then([this, &iov] (size_t size) {
        internal::iovec_trim_front(iov, size);
        return iov.empty() ? make_ready_future<>() : write_all(iov);
    });
}
#else
