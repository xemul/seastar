    ...
    struct io_queue::config generate_config(dev_t devid, unsigned nr_groups) const {
        ...
        if (!_capacity) {
            ...
            for (auto&& st : p.read_bytes_steps) {
                ...
                cfg.disk_bytes_read_multiplier.push(st.size, (io_queue::read_request_base_count * p.read_bytes_rate) / st.bandwidth);
            }
            for (auto&& st : p.write_bytes_steps) {
                ...
                cfg.disk_bytes_write_multiplier.push(st.size, (io_queue::read_request_base_count * p.read_bytes_rate) / st.bandwidth);
            }
