...
reactor::task_queue::register_stats() {
    ...
    new_metrics.add_group("scheduler", {
        ...
        sm::make_gauge("shares", [this] { return _shares; },
                ...
                {group_label}),
        sm::make_absolute("time_spent_on_task_quota_violations_ms", [this] {
...
cpu_stall_detector::cpu_stall_detector(cpu_stall_detector_config cfg)
    ...
    _metrics.add_group("stall_detector", {
            sm::make_absolute("reported", _total_reported, sm::description("Total number of reported stalls, look in the traces for the exact reason"))});
...
void reactor::register_metrics() {
    ...
    _metric_groups.add_group("reactor", {
            ...
            sm::make_absolute("tasks_processed", std::bind(&reactor::tasks_processed, this), sm::description("Total tasks processed")),
            sm::make_absolute("polls", _polls, sm::description("Number of times pollers were executed")),
            sm::make_counter("timers_pending", std::bind(&decltype(_timers)::size, &_timers), sm::description("Number of tasks in the timer-pending queue")),
            sm::make_gauge("utilization", [this] { return (1-_load)  * 100; }, sm::description("CPU utilization")),
            sm::make_absolute("cpu_busy_ms", [this] () -> int64_t { return total_busy_time() / 1ms; },
                    sm::description("Total cpu busy time in milliseconds")),
            sm::make_absolute("cpu_steal_time_ms", [this] () -> int64_t { return total_steal_time() / 1ms; },
            ...
            // total_operations value:DERIVE:0:U
            sm::make_absolute("aio_reads", _io_stats.aio_reads, sm::description("Total aio-reads operations")),

            sm::make_total_bytes("aio_bytes_read", _io_stats.aio_read_bytes, sm::description("Total aio-reads bytes")),
            // total_operations value:DERIVE:0:U
            sm::make_absolute("aio_writes", _io_stats.aio_writes, sm::description("Total aio-writes operations")),
            sm::make_total_bytes("aio_bytes_write", _io_stats.aio_write_bytes, sm::description("Total aio-writes bytes")),
            sm::make_absolute("aio_errors", _io_stats.aio_errors, sm::description("Total aio errors")),
            // total_operations value:DERIVE:0:U
            sm::make_absolute("fsyncs", _fsyncs, sm::description("Total number of fsync operations")),
            // total_operations value:DERIVE:0:U
            sm::make_absolute("io_threaded_fallbacks", std::bind(&thread_pool::operation_count, _thread_pool.get()),
                    sm::description("Total number of io-threaded-fallbacks operations")),

    });

    _metric_groups.add_group("memory", {
            sm::make_absolute("malloc_operations", [] { return memory::stats().mallocs(); },
                    sm::description("Total number of malloc operations")),
            sm::make_absolute("free_operations", [] { return memory::stats().frees(); }, sm::description("Total number of free operations")),
            sm::make_absolute("cross_cpu_free_operations", [] { return memory::stats().cross_cpu_frees(); }, sm::description("Total number of cross cpu free")),
            ...
            sm::make_absolute("reclaims_operations", [] { return memory::stats().reclaims(); }, sm::description("Total reclaims operations"))
    });

    _metric_groups.add_group("reactor", {
            sm::make_absolute("logging_failures", [] { return logging_failures; }, sm::description("Total number of logging failures")),
            // total_operations value:DERIVE:0:U
            sm::make_absolute("cpp_exceptions", _cxx_exceptions, sm::description("Total number of C++ exceptions")),
            sm::make_absolute("abandoned_failed_futures", _abandoned_failed_futures, sm::description("Total number of abandoned failed futures, futures destroyed while still containing an exception")),
    });
    ...
    _metric_groups.add_group("reactor", {
        ...
        make_absolute("fstream_read_bytes", _io_stats.fstream_read_bytes,
                description(
        ...
        make_absolute("fstream_read_bytes_blocked", _io_stats.fstream_read_bytes_blocked,
                description(
        ...
        make_absolute("fstream_reads_ahead_bytes_discarded", _io_stats.fstream_read_ahead_discarded_bytes,
                description(
...
void smp_message_queue::start(unsigned cpuid) {
    ...
    _metrics.add_group("smp", {
            ...
            // total_operations value:DERIVE:0:U
            sm::make_absolute("total_received_messages", _received, sm::description("Total number of received messages"), {sm::shard_label(instance)})(sm::metric_disabled),
            // total_operations value:DERIVE:0:U
            sm::make_absolute("total_sent_messages", _sent, sm::description("Total number of sent messages"), {sm::shard_label(instance)})(sm::metric_disabled),
            // total_operations value:DERIVE:0:U
            sm::make_absolute("total_completed_messages", _compl, sm::description("Total number of messages completed"), {sm::shard_label(instance)})(sm::metric_disabled)
    });
}
