...
public:
    ...
    virtual future<size_t> sendmsg(pollable_fd_state& fd, std::span<iovec> iovs, size_t iovlen) final {
        ...
        class write_completion final : public io_completion {
            ...
            promise<size_t> _result;
        public:
            write_completion(pollable_fd_state& fd, std::span<iovec> iovs, size_t iovlen)
                : _fd(fd), _to_write(iovlen) {
                _mh.msg_iov = iovs.data();
                _mh.msg_iovlen = iovs.size();
            }
            void complete(size_t bytes) noexcept final {
                if (bytes == _to_write) {
        ...
        auto desc = std::make_unique<write_completion>(fd, iovs, iovlen);
