...
output_stream<char> content_replace::transform(std::unique_ptr<request> req,
    ...
    return output_stream<char>(content_replace_data_sink(std::move(s), {std::make_tuple("Protocol", protocol), std::make_tuple("Host", host)}), 32000, stream_trim_to_size::yes);
