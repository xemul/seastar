...
int main(int ac, char** av) {
    ...
    at.add_options()
            ...
            ("nr_queues", bpo::value<unsigned>()->default_value(4), "Number of queues to simulate")
            ("iops", bpo::value<unsigned>()->default_value(10000), "IOPS")
            ("bandwidth_mb", bpo::value<unsigned>()->default_value(100), "Bandwidth in MB/s")
            ("concurrency", bpo::value<unsigned>()->default_value(4), "Concurrency of competing low-prio group")
            ("fg_concurrency", bpo::value<unsigned>()->default_value(1), "Concurrency of foreground group (burst size)")
            ("poll_interval_us", bpo::value<unsigned>()->default_value(250), "Interval between queue polls")
        ;

    return at.run(ac, av, [&at] {
        auto nr_iterations = at.configuration()["iterations"].as<unsigned>();
        auto nr_queues = at.configuration()["nr_queues"].as<unsigned>();
        auto iops = at.configuration()["iops"].as<unsigned>();
        auto bandwidth_mb = at.configuration()["bandwidth_mb"].as<unsigned>();
        auto bg_concurrency = at.configuration()["concurrency"].as<unsigned>();
        auto fg_concurrency = at.configuration()["fg_concurrency"].as<unsigned>();
        auto poll_interval = microseconds(at.configuration()["poll_interval_us"].as<unsigned>());
